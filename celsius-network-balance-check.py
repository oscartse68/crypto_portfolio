...
class CelsiusNetworkAPI:
    ...
    def request(self, command: str, **kwargs):
        if command not in self.__actions.keys():
            ...
            return requests.get(self.__base_url+self.__actions[command]['url'], headers=self.headers)

    def get_cel_historical_interest(self, coin_list: list, start_date: str, end_date: str) -> pd.DataFrame:
        __celsian_stata_url = "https://raw.githubusercontent.com/Celsians/Google-Sheets/master/data.json"
        response = requests.request("GET", __celsian_stata_url)
